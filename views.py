...
def render_classification(request):
    ...
    model = pickle.load(open("C:\\Users\\Rehan Khan\\Downloads\\Credit-Limit_Optimiser\\cluster_model", 'rb'))
...
def render_prediction(request):
    ...
    for i in ["Avg_Utilization_Ratio", "Pay_on_time", "Income_Category", "Months_on_book" ,"Education_Level", "Card_Category"]:
        ...
        if i == "Card_Category":
            encoder1 = pickle.load(open("C:\\Users\\Rehan Khan\\Downloads\\Credit-Limit_Optimiser\\card_cat", 'rb'))
            data.append(encoder1.transform(np.array(request.POST[i]).reshape(1, -1)).toarray()[0][0])
            continue

        if i == "Education_Level":
            encoder2 = pickle.load(open("C:\\Users\\Rehan Khan\\Downloads\\Credit-Limit_Optimiser\\ed_lvl", 'rb'))
    ...
    encoder3 = pickle.load(open("C:\\Users\\Rehan Khan\\Downloads\\Credit-Limit_Optimiser\\feature_scaler", 'rb'))
    data = encoder3.transform(np.array(data).reshape(1, -1))
    model = pickle.load(open("C:\\Users\\Rehan Khan\\Downloads\\Credit-Limit_Optimiser\\predictor_model", 'rb'))
    scaler = pickle.load(open("C:\\Users\\Rehan Khan\\Downloads\\Credit-Limit_Optimiser\\target_scaler", 'rb'))
